...
class AutoTrader:
    def __init__(self, args):
        ...
        self.deposit_wallet: Optional[str] = None  # Computed from private key
...
""")
    ...
    def _init_live(self):
        ...
        # 3. Fetch real balance via raw L2-authed HTTP
        try:
            from web3 import Web3
            import hmac, hashlib, base64
            
            wallet_addr = Web3().eth.account.from_key(self._private_key).address
            self.deposit_wallet = compute_deposit_wallet_address(wallet_addr)
            print(f"🏦 Deposit Wallet: {self.deposit_wallet}")
            
            # HMAC sign the balance request
            ts = str(int(time.time() * 1000))
            message = ts + "GET" + "/balance-allowance" + ""
            raw_secret = self.clob_creds["api_secret"]
            ...
            headers = {
                "POLY-API-KEY": self.clob_creds["api_key"],
                "POLY-TIMESTAMP": ts,
                "POLY-SIGNATURE": sig,
                "POLY-PASSPHRASE": self.clob_creds["api_passphrase"],
            }
            ...
            poly_balance = float(data.get("balance", 0))
            if poly_balance > 0:
                print(f"💰 Polymarket balance: ${poly_balance:,.2f} — bankroll updated")
                self.bankroll = poly_balance
                self.initial_bankroll = poly_balance
            else:
                wallet_balance = fetch_usdc_balance(wallet_addr)
                if wallet_balance > 0:
                    print(f"💰 Wallet USDC: ${wallet_balance:.2f} — bankroll updated")
                    self.bankroll = wallet_balance
                    self.initial_bankroll = wallet_balance
                else:
                    print(f"⚠️  No balance found — using --bankroll ${self.bankroll:.2f}")
                    self.initial_bankroll = self.bankroll
        except Exception as e:
            # Balance check non-fatal — use --bankroll value
